...
rabbithole eval benchmark [file] [--model M] [--provider P] [--host URL] [--no-think]
                          [--repeats N] [--format markdown|json] [--output-path PATH]
```
...
**The report explains each number as it prints it**, so none of this needs memorising.

**Check the spread of marks first.** Everything else depends on it.

```
Scores the model used: 0×7 1×10 2×6 3×4 4×6 5×2 (6 different, against 11 in golden)
```
...
**The four headline numbers.** In plain words:

| | |
|---|---|
| score agreement | Whether its marks relate to yours at all, once you discount lucky guesses. 1.0 is perfect, **0.0 means no relationship was found**, below 0 means it disagrees systematically. |
| average miss | How far off it is, in points. |
| large misses | Always at least the average. The further above, the more a few bad articles account for the damage. |
| score offset | Which way it leans. A big number here means it is consistently too generous or too harsh, which is a different problem from being scattered. |
...
3+    12      27      0.67       0.77    0.30
4+    8       23      0.62       0.66    0.22
5+    2       17      0.00       0.49    0.00
6+    0       14      -          0.40    0.00
7+    0       12      -          0.34    0.00
8+    0       10      -          0.29    0.00
9+    0       7       -          0.20    0.00
...
will not help. Above, the keyword matcher is *below* chance everywhere, the right verdict for
a keyword matcher tested on a set built to catch one.
...
```
TAG                      N   MAE   SIGNED  
clickbait-framing        3   8.00  -8.00   clickbait headline over a real article
on-target                7   7.71  -7.71   what you most want to read
deep-but-off-topic       3   1.33  -1.33   excellent, wrong domain
```
...
**Agreement and Disagreements** list what it got right and what it got most wrong, furthest
first, with its reasoning beside your note. That pairing is what makes a failure fixable: you
can usually see which line it followed. Read Agreement alongside the spread above, though: a
long list built from one repeated mark is a warning, not a result.
...
| Score agreement near 0.0 | Its marks bear no relation to yours | Bigger model, before touching your profile |
| Large offset, misses all one way | Right order, wrong level | Reword your tiers |
| One tag much worse than the rest | It's wrong about that kind of article | Read that tag's Disagreements |
...
```
profile 07377fd54469 · prompt 351a9edc8435 · benchmark 070506edf6e0
```
