// Package server exposes ai-searcher's item store over HTTP as JSON.
// Handlers call the same internal/store functions the CLI calls; HTML
// rendering is a later, purely additive phase layered on top of
// parseListFilter/store.List without changing either.
package server
...
// Server wires HTTP handlers to a Store and the config that supplies their
// defaults (e.g. the default list window width).
type Server struct {
	db  *store.Store
	cfg *config.Config
}
...
// Routes builds the HTTP handler. Uses the stdlib's method+wildcard
// ServeMux (Go 1.22+) — no router dependency needed.
